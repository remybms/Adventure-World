            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "20px" }}>
                {skills.map((skill) => (
                    <div key={skill.id} style={{ border: "1px solid #ccc", padding: "15px", width: "300px", borderRadius: "8px" }}>
                        <h3>{skill.nom}</h3>
                        <p><strong>Classe :</strong> {skill.classeRequise}</p>
                        <p><strong>Niveau Min :</strong> {skill.niveauMinimum}</p>
                        <p>{skill.description}</p>
                        {skill.competencesRequises && skill.competencesRequises.length > 0 && (
                            <small>Prérequis : {skill.competencesRequises.length} compétence(s)</small>
                        )}
                    </div>
                ))}
            </div>
